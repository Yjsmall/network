...
{
  // 1UL << 32 is the max of 32bits + 1, which isn't belong uint32_t,
  // but after 'mod' operation, the result will be in this interval.
...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  if ( this->raw_value_ >= zero_point.raw_value_ ) {
    ...
    raw = ( static_cast<uint64_t>( this->raw_value_ ) + ( 1UL << 32 )
  ...
  uint64_t probable_res3
    = probable_res2 < ( 0xFFFF'FFFF'FFFF'FFFF ) - ( 1UL << 32 ) ? probable_res2 + ( 1UL << 32 ) : probable_res2;
